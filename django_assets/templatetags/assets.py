...
def create_merged(sources, output, filter, force=False):
    ...
    If ``force`` is enabled, the resources will always be recreated, and
    the check, whether an update is required, is skipped.
    """
    ...
    # check if the asset needs to be (re)created
    ...
    return os.stat(output_path).st_mtime
...
class AssetsNode(template.Node):
    ...
    def render_merged(self, context):
        ...
        last_modified = create_merged(files, output, filter)
