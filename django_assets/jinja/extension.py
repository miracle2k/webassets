...
class AssetsExtension(Extension):
    ...
    def parse(self, parser):
        ...
        while parser.stream.current.type is not 'block_end':
            # string constants need to be handled separately, since
            # parse_expression() would fold multiple whitespace-separated
            # ones into a single node.
            if parser.stream.current.test('string'):
                files.append(nodes.Const(parser.stream.next().value))
            else:
